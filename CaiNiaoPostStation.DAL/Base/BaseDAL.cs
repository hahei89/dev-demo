...
namespace CaiNiaoPostStation.DAL.Base
{
    public  class BaseDAL<T>:BQuery<T> where T:class
    {
        ...
        public int Add(T model, string cols, int isReturn)
        {
            ...
            SQLiteParameter[] paras = insert.Paras;
            if (isReturn == 0)
        ...
        /// <returns></returns>
        public bool Update(T model, string cols, string strWhere, params SQLiteParameter[] paras)
        {
            if (model == null)
                return false;
            SqlModel update = CreateSql.CreateUpdateSql<T>(model, cols, strWhere);
            string sql = update.Sql;
            SQLiteParameter[] paras0 = update.Paras;
            List<SQLiteParameter> listParas = paras0.ToList();
        ...
        /// <returns></returns>
        public bool Delete(int delType, string strWhere, int isDeleted, params SQLiteParameter[] paras)
        {
            string delSql = "";
            if (delType == 0)
        ...
        private List<CommandInfo> GetCommandList(string sql, bool isProc, params SQLiteParameter[] paras)
        {
            List<CommandInfo> comList = new List<CommandInfo>();
